...
class ChiqAirCon : public climate::Climate, public Component, public uart::UARTDevice {
 public:
  ...
  void set_custom_fan_modes(std::initializer_list<const char *> modes) {
    ...
    for (auto *mode : modes)
      this->supported_custom_fan_modes_.emplace(mode);
  }
 ...
 protected:
  ...
  void parse_status_payload_(const uint8_t *payload) {
    ...
    this->fan_mode = this->decode_fan_(payload[7]);
    ...
    if (payload[6] & 0x08)
      ...
      this->custom_fan_mode = Constants::QUIET;
  ...
  void send_command_() {
    ...
    payload[7] = this->encode_fan_(this->fan_mode);
  ...
  uint8_t encode_special_() const {
    ...
    if (this->custom_fan_mode == Constants::TURBO)
      special |= 0x08;
    if (this->custom_fan_mode == Constants::QUIET)
      special |= 0x04;
    return special;
  }
  ...
  climate::ClimateModeMask supported_modes_{0};
  climate::ClimateSwingModeMask supported_swing_modes_{0};
  climate::ClimatePresetMask supported_presets_{0};
  std::set<std::string> supported_custom_fan_modes_{Constants::TURBO, Constants::QUIET};
